...
<div class="list" data-field-name="{{ $field['name'] }}">
    ...
    @if ($multiple)
        <input type="hidden" data-marker="multipleBrowseInput" name="{{ $field['name'] }}" value="{{ old(square_brackets_to_dots($field['name'])) ?? json_encode($value) }}">
    @else
        <input type="text" data-marker="multipleBrowseInput" name="{{ $field['name'] }}" value="{{ old(square_brackets_to_dots($field['name'])) ?? $value }}" @include('crud::fields.inc.attributes') readonly>
    @endif
</div>
<div class="input-group-append d-flex justify-content-end mt-1" role="group" aria-label="..." style="margin-top: 3px;">
...
<script type="text/html" data-marker="browse_multiple_template">
    <div class="input-group input-group-sm">
        ...
        <div class="input-group-btn">
            ...
            @if($sortable)
                <button type="button" class="browse move btn btn-sm btn-light">
